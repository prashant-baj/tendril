...
|-----:|------|----|-------|:------:|------------|--------------|
| 1 | PG | PG-06 | Decoupled deploy & CI for prompts and guardrails | ☐ | PG-03, TF-07 | CI change-detection watches only `infra/**`,`agents/**`; deploy line ships only foundation+agentcore. Add `prompts/`+`guardrails/` filters + deploy `tendril-<env>-prompts` & `-guardrails`; document cold-env deploy order. |
...
| 5 | TF | TF-07 | CI/CD & selective deploy (GitOps) | ◐ | TF-03 | PR lint/test/gitleaks + OIDC + path deploy to dev all working. **Gaps:** no prod deploy path (tag/approval gate); prompts/guardrails wiring tracked under PG-06; branch protection is account-side (unverifiable from repo). |
