...
| 20 | TR/SR | TR-01..03, SR-01..03 | Real session-resume + tracker/scheduler + `TasksDueIndex` follow-ups | ✅ | AF-05, PA-02 | `docs/stories/tracker-scheduler.md` (new epic, `docs/roadmap.md` Phase 7.5+ — all six stories done). **TR-01/02/03**: `TasksDueIndex` GSI + due-date stamping (`approvePlan`)/clearing (`postTaskCheckin`); new `app/tracker/` Lambda (EventBridge Scheduler, 15-min rate) querying the GSI and emitting `followup.due`; orchestrator's new `handle_followup_due` (deliberately model-free — a deterministic nudge message + Activity event + due-date bump, not a full agent turn). New CDK assertion test enforces ADR-0013 per-specialist-role (not aggregated) — closes `data-architecture.md` §9's long-open item 6. **Found + fixed a real bug live:** the tracker's `dynamodb:Query` against `TasksDueIndex` 403'd — `Table.from_table_name()`'s `grant_read_data()` only grants base-table actions, never `.../index/*`; fixed with an explicit IAM statement naming the index ARN, plus a regression-guarding CDK test. **SR-01/02/03**: provisioned `tendril-{env}-agent-state` (S3, orchestrator-only IAM); wired a real `SnapshotSessionManager` (`session_id=goal_id`) into `_build_orchestrator_agent`, replacing the full-DynamoDB-reconstruction-every-turn pattern PA-02 deliberately deferred; a hand-rolled `_strip_trailing_unresolved_tool_use` (no such function exists in the installed `strands-agents` SDK) guards resumed history; each handler's prompt is now just the genuinely new information for that turn (the latest message, or a fixed check-in framing), not a full transcript restatement — `_format_transcript`/`_load_messages` deleted. Deployed to `dev` and live-verified end to end, both halves: approved a real plan (due-dates stamped), manually triggered the tracker (0 due, correctly), backdated one task, re-triggered (published `followup.due`, confirmed a deterministic nudge message + Activity event + due-date bump with **no** model call), checked the task in (confirmed GSI attributes removed); separately, submitted a goal (confirmed a session snapshot appeared in `tendril-dev-agent-state` keyed by `goal_id`) and sent a follow-up chat message (confirmed the snapshot was updated and the reply stayed coherent, correctly referencing the prior turn without a restated transcript). |
